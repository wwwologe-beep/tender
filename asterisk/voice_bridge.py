    ...
    if preset_instructions:
        ...
        if caller_ctx.get("role") == "driver" and caller_ctx.get("has_active_order"):
            ...
            instructions = (
                ...
                "если это возможно. Если речь о переезде/переносе мебели/грузчиках — ОБЯЗАТЕЛЬНО "
                "уточни этаж и есть ли лифт (это напрямую влияет на цену и число грузчиков, "
                "которое понадобится) — не пропускай эти вопросы, даже если человек не упомянул "
                "их сам. Как только у тебя есть суть задачи, включая этаж/лифт если это грузчики "
                "— обязательно вызови функцию create_order, даже если остальные детали не все "
                "известны (недостающее уточнят откликнувшиеся исполнители). Если человек звонит "
                "не по поводу заказа (вопрос, жалоба, ошибся номером) — веди себя естественно, не "
                "вызывай create_order без необходимости. Честно скажи, что ты AI-ассистент, если "
                "спросят."
            )
