...
**Входящий звонок** (кто угодно звонит на 2115325): **с 22.07.2026 создаёт реальный
заказ.** `handle_call()` различает входящий звонок (нет `attempt_id`, нет
`preset_instructions`) от звонков оркестратора, даёт ему intake-промпт и единственный tool
`create_order`. Номер звонящего берётся из ARI `channel.caller.number` (нормализуется с
`+`) и передаётся в новый роут `POST /api/orchestrator/create-order-from-call` —
переиспользует `analyzeOrder()` и тот же fan-out (Telegram push, voice-orchestrator
очередь), что и обычное создание заказа с сайта. **Не протестировано реальным входящим
звонком** — код задеплоен и на Vercel, и на VPS (`voice_bridge.py` перезапущен), но живой
