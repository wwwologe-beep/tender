...
## 2a. Миграция Vercel → Railway (технически завершена 23.07.2026, не подтверждена сквозным тестом)
...
**Railway-проект:** workspace `heroic-recreation`, **два сервиса** в одном проекте:
- `tender` — сайт + все API, подключён к `wwwologe-beep/tender` (ветка `main`).
- `tender-cron-ping` — тот же репозиторий, но с Custom Build Command `echo "no build
  needed"` (пропускает `next build`, не нужен для простого Node-скрипта) и Custom Start
  Command `node scripts/railway-cron-ping.js`.

Прод-URL: `https://tender-production-26c5.up.railway.app`.

**Технические детали, стоившие времени на отладку:**
1. Next.js на Railway (Railpack builder) слушает порт **8080** по умолчанию (не 3000) —
   домен в Networking должен указывать на 8080, иначе `502 Application failed to respond`,
   даже если сборка прошла успешно.
2. Второй сервис (`tender-cron-ping`) по умолчанию тоже пытается собраться как обычный
   Next.js-проект (`npm run build`) и падает без нужных env-переменных — Custom Start
   Command сам по себе не отменяет билд-шаг, нужен отдельно Custom Build Command.
3. `CRON_SECRET` нужно задать **в обоих сервисах** — и в `tender` (чтобы `cron/tick`
   вообще принимал запросы), и в `tender-cron-ping` (чтобы было что слать) — забыть про
   один из двух означает тихий 401 несмотря на видимо успешный деплой.

**Статус по компонентам:**
| Компонент | Статус |
|---|---|
| Основной сервис `tender` | ✅ Деплоится, отвечает 200 |
| Все ~21 env-переменная перенесены | ✅ (3 "Sensitive"-переменные Vercel пришлось пересоздать заново, а не скопировать — Vercel не показывает их значение никому после создания) |
| `ORCHESTRATOR_BRIDGE_SECRET` | ✅ Пересоздан заново, синхронизирован Railway ↔ VPS (`/root/.voice_bridge.env`) |
| `CRON_SECRET` (в обоих Railway-сервисах) | ✅ Добавлен впервые за всё время существования проекта — до этого `GET /api/cron/tick` всегда отвечал 401 любому вызывающему, код требовал секрет, которого нигде не существовало |
| Telegram webhook | ✅ Переключён на Railway. Побочная находка: до этого бот указывал на мёртвый ngrok-туннель (404) — не работал вообще, независимо от миграции. После переключения разом доставлена накопленная очередь сообщений (`pending_update_count` был >0, ожидаемо, сейчас 0) |
| Wappi (WhatsApp) webhook | ✅ Переключён на Railway (app.wappi.pro dashboard) |
| VPS `voice_bridge.py` → `NEXTJS_APP_URL` | ✅ Переключён на Railway, процесс перезапущен |
| `tender-cron-ping` (частый cron) | ✅ Задеплоен, статус Active. `GET /api/cron/tick` с правильным secret вручную подтверждён 200 |
| Vercel (`tender-navy.vercel.app`) | Остаётся как резерв, не отключён |

**Не подтверждено:**
1. Что `tender-cron-ping` реально пингует раз в минуту непрерывно (не только единоразовая
   ручная проверка) — логи самого сервиса на регулярные вызовы не проверялись.
2. Сквозной тест на новом окружении — реальный звонок/заказ, подтверждающий что обзвон
   стартует за ~1 минуту, а не ~10, как было на Vercel.
